...
void findlabelMap() { // Find all the target for labelMap from the jump statements
	...
	for (vector<quad>::iterator it = array.begin(); it!=array.end(); it++) {
		...
		if(op=="PARAM"){
			...
			if (op=="ADD") {
				...
				if (flag) asmfile << "\timull \t$" << atoi(arg2.c_str()) << ", " << "%eax" << endl;
			...
			// Unary Operators
				...
				asmfile << "\tmovq\t" << table->ar[arg1] << "(%rbp,%rax,1), "<<"%rax" << endl;
				asmfile << "\tmovq \t%rax, " <<  table->ar[result] << "(%rbp)";
			}	 			
			else if (op=="ARRL") {
				asmfile << "movq\t" << table->ar[arg1] << "(%rbp), "<<"%rax" << endl;
				asmfile << "\tmovq\t" << table->ar[arg2] << "(%rbp), "<<"%rdx" << endl;
				asmfile << "\tmovq\t" << "%rdx, " << table->ar[result] << "(%rbp,%rax,1)";
			}	 
			else if (op=="RETURN") {
